...
class XAccountCleanerLogLine(models.Model):
    ...
    def _account_exists_in_chart(self):
        ...
        company_field = self._get_account_company_field()
        if self.company_id and company_field == 'company_id':
            domain.append(('company_id', '=', self.company_id.id))
        elif self.company_id and company_field == 'company_ids':
    ...
    def action_restore_lines(self):
        ...
        for line in self:
            ...
            company_field = line._get_account_company_field()
            if company_id and company_field == 'company_id':
                duplicate_domain.append(('company_id', '=', company_id))
            elif company_id and company_field == 'company_ids':
    ...
    def _prepare_account_create_vals(self, snapshot):
        self.ensure_one()
        target_company_id = self._get_target_company_id(snapshot)
        account_type_field = self._get_account_type_field()
        account_type_value = snapshot.get('account_type') or self.account_type or 'expense'
        vals = {
        ...
        if company_field == 'company_id':
            ...
            vals['company_ids'] = [(6, 0, [target_company_id] if target_company_id else [])]

        if account_type_field == 'account_type':
            vals['account_type'] = account_type_value
        elif account_type_field == 'user_type_id':
            vals['user_type_id'] = self._resolve_user_type_id(snapshot, target_company_id)

        if account_type_value in ('asset_receivable', 'liability_payable', 'receivable', 'payable'):
            vals['reconcile'] = True
        if account_type_value == 'off_balance':
    ...
    def _get_account_type_field(self):
        account_fields = self.env['account.account']._fields
        if 'account_type' in account_fields:
            return 'account_type'
        if 'user_type_id' in account_fields:
            return 'user_type_id'
        return False

    def _resolve_user_type_id(self, snapshot, company_id=False):
        user_type_id = snapshot.get('user_type_id')
        if user_type_id:
            return user_type_id

        account_type_value = snapshot.get('account_type') or self.account_type
        if account_type_value in ('asset_receivable', 'receivable'):
            target_type = 'receivable'
        elif account_type_value in ('liability_payable', 'payable'):
            target_type = 'payable'
        elif account_type_value in ('asset_cash', 'liquidity', 'liability_credit_card'):
            target_type = 'liquidity'
        else:
            target_type = 'other'

        account_type_model = self.env['account.account.type']
        user_type = account_type_model.search([('type', '=', target_type)], limit=1)
        if not user_type and target_type != 'other':
            user_type = account_type_model.search([('type', '=', 'other')], limit=1)

        if user_type:
            return user_type.id

        fallback_domain = []
        if company_id and 'company_id' in account_type_model._fields:
            fallback_domain = ['|', ('company_id', '=', False), ('company_id', '=', company_id)]
        fallback = account_type_model.search(fallback_domain, limit=1)
        return fallback.id if fallback else False

    def _get_account_company_field(self):
        account_fields = self.env['account.account']._fields
    ...
    def _get_target_company_id(self, snapshot):
        self.ensure_one()
        snapshot_company_id = snapshot.get('company_id')
        if snapshot_company_id:
            return snapshot_company_id
        snapshot_company_ids = snapshot.get('company_ids') or []
        if snapshot_company_ids:
            return snapshot_company_ids[0]
        if self.company_id:
