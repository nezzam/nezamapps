...
class AccountCleaner(models.Model):
    ...
    def _get_account_type_value(self, account):
        account_fields = account._fields
        if 'account_type' in account_fields:
            return account.account_type or False
        if 'user_type_id' in account_fields and account.user_type_id:
            return account.user_type_id.type or False
        return False
    ...
    def _build_account_snapshot(self, account):
        ...
        fields_to_capture['account_type'] = self._get_account_type_value(account)
        if 'user_type_id' in account_fields:
            fields_to_capture['user_type_id'] = account.user_type_id.id if account.user_type_id else False
    ...
    def _check_if_protected_simple(self, account):
        ...
        try:
            ...
            # Check protected account types (version-safe for Odoo 15+)
            ...
            account_type_value = self._get_account_type_value(account)
            if account_type_value in protected_account_types:
                return True, _('System account type: %s', account_type_value)
